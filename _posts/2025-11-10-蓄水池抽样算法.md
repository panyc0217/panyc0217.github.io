---
description: "深入解析蓄水池抽样算法：问题背景、算法实现、数学证明、工程实践（红包分配、权重抽奖），附 Go 语言完整实现"
categories: [ 数据结构与算法]
tags: [ 数据结构与算法 , 随机算法 ]
published: true # 是否公开
comment: true # 评论区
...
> 设 $i$ 为元素在数据流中的编号，$v$ 为具体元素
> 1. 当 $i < m$ 时，直接将元素 $v$ 放入样本集的 $i$ 号位置
> 2. 当 $i \geq m$ 时，取随机数 $d \in \left[ 0, i \right]$ ，若 $d < m$ 则将样本集的 $d$ 号位置元素更新为 $v$
